...
-- 创建应用用户（可选）
-- CREATE USER 'meituan_app'@'localhost' IDENTIFIED BY 'your_password';
-- GRANT SELECT, INSERT, UPDATE, DELETE ON meituan_product.* TO 'meituan_app'@'localhost';
-- FLUSH PRIVILEGES;
...
/*
1. 数据库字符集：utf8mb4，支持 emoji 和特殊字符
2. 排序规则：utf8mb4_unicode_ci，不区分大小写
3. 存储引擎：InnoDB，支持事务和外键
4. 时间字段：使用 DATETIME 类型，自动更新
5. 索引：为常用查询字段创建索引，提高查询性能

使用方法：
1. 连接到 MySQL 服务器
2. 执行此脚本：mysql -u root -p < database-init.sql
3. 或在 MySQL 客户端中：source database-init.sql
